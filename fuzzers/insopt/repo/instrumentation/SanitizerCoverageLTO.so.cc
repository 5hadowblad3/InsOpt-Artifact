...
class ModuleSanitizerCoverageLTO
 ...
 private:
  ...
  void buildGlobalCFG(Module &M) {
    GlobalCFG.Preds.clear();
    GlobalCFG.Succs.clear();
    // 对每个函数遍历所有BasicBlock
    for (Function &F : M) {
      if (F.isDeclaration()) continue;
      for (const BasicBlock &BB_ref_obj : F) { // BB_ref_obj: const BasicBlock&
        const BasicBlock *currentBB_ptr = &BB_ref_obj;

        GlobalCFG.Preds[currentBB_ptr];
        GlobalCFG.Succs[currentBB_ptr];

        for (const BasicBlock *Succ_ptr : successors(currentBB_ptr)) {
          GlobalCFG.Succs[currentBB_ptr].push_back(Succ_ptr);
          GlobalCFG.Preds[Succ_ptr].push_back(currentBB_ptr);
        }
      }
    }
  }
  ...
  void calculateGlobalNodeOutputEntropies(Module &M) {
    ...
    for (Function &F : M) {
        ...
        for (const BasicBlock &BB_ref : F) {
            ...
            nodeOutputEntropies[BB_ptr] = 0; // 初始化熵
    ...
    // 2. Iterative Fixed-Point Calculation
    while (!worklist.empty()) {
        ...
        // Get predecessor and successor info strictly from GlobalCFG
        // .lookup() returns a default-constructed SmallVector (empty) if key not found.
        ...
        // Rule 1: 1 pred, 1 succ => Entropy = Entropy of Predecessor
            ...
            // predecessorsVec is guaranteed to have one element if num_preds is 1.
            const BasicBlock *P = predecessorsVec[0];
            new_entropy = nodeOutputEntropies.lookup(P); // lookup P's entropy
        }
        ...
        // Other cases (e.g., 0 preds/0 succs, 0 preds/1 succ, 1 pred/0 succs)
        // will not match these rules, so new_entropy remains old_entropy (typically 0 from init).

        // Check for changes and update worklist
        if (new_entropy != old_entropy) {
            nodeOutputEntropies[currentBB] = new_entropy; // Update the entropy

            // Add all successors of currentBB to the worklist
            for (const BasicBlock *Succ : successorsVec) {
                if (worklistSet.insert(Succ).second) { // If Succ was not in worklistSet
                    worklist.push_back(Succ);          // Add it to the physical worklist
                }
            }
        }
    }
  ...
  // 使用原有的局部规则计算每个基本块的entropy，算法不变
  /* int computeLocalEntropy(const BasicBlock *BB) {
    // 如果为入口块，entropy为0
    const Function *F = BB->getParent();
    if (BB == &F->getEntryBlock())
      return 0;
    int predCount = GlobalCFG.Preds.lookup(BB).size();
    int succCount = GlobalCFG.Succs.lookup(BB).size();
    int entropy = 0;
    if (predCount == 1 && succCount == 1) {
      const BasicBlock *Pred = GlobalCFG.Preds.lookup(BB)[0];
      entropy = computeLocalEntropy(Pred);
    } else if (predCount == 1 && succCount > 1) {
      const BasicBlock *Pred = GlobalCFG.Preds.lookup(BB)[0];
      entropy = computeLocalEntropy(Pred) + 1;
    } else if (predCount > 1 && succCount == 1) {
      entropy = 0;
    } else if (predCount > 1 && succCount > 1) {
      entropy = 1;
  ...
  // 计算全模块每个BB的entropy，利用LTO全局CFG信息迭代计算（本例使用一次遍历）
  /*  void calculateGlobalEntropy() {
    // 遍历GlobalCFG中所有BB，并计算其entropy
    for (auto &Entry : GlobalCFG.Preds) {
      const BasicBlock *BB = Entry.first;
      int entropy = computeLocalEntropy(BB);
      GlobalEntropy[BB] = entropy;
    }
  }*/
  ...
  for (auto &BB : F) {
    ...
    if (num_actual_preds > 1) {
            ...
            for (const BasicBlock *P : current_preds_vec_for_stats) {
                sum_incoming_entropies += nodeOutputEntropies.lookup(P); // lookup is safe
            }
...
//ADDING ENTROPY
void ModuleSanitizerCoverageLTO::printFinalStats() {
  ...
  for (unsigned i = 0; i <= 10; ++i) { // 迭代0到10 (0代表0个前驱, 10代表10个及以上)
    unsigned count = InstPredCountsHistogram.lookup(i); // lookup 不存在则返回0
