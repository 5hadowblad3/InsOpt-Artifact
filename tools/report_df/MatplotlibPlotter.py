...
def expected_time_to_trigger(bd, outdir):
    ...
    annotations[fuzzer_label] = annotations[fuzzer_label].applymap(lambda x: pp_time_second(x))
...
def unique_bugs_per_target(bd, outdir, metric, libraries=None, symmetric=False, ncols=3):
    ...
    '''# 修改 error bar 部分，填充 NaN 值为 0
    fig, ax = plt.subplots(figsize=(12, 6))
    mean_df = unique_bugs.groupby(['Fuzzer','Target']).mean().unstack(0)
    std_df = unique_bugs.groupby(['Fuzzer','Target']).std().unstack(0).fillna(0)
    mean_df.plot.bar(width=0.8, ax=ax, yerr=std_df)
...
def bug_metric_boxplot(bd, outdir):
    ...
    def plot_boxes(df):
        ...
        bug_labels = list(df.columns)

        # 计算每个 bug 的最早出现时间，并更新 y tick label
        new_bug_labels = []
        for bug in bug_labels:
            times = df[bug].dropna()
            if not times.empty:
                appear_time = times.min()
                new_label = f"{bug} ({pp_time_second(appear_time)})"
            else:
                new_label = bug
            new_bug_labels.append(new_label)
        ax.set_yticklabels(new_bug_labels)
    ...
    def plot_target_program_bug(series):
        ...
        for ((fuzzer, metric), kmf) in series.items():
            ...
            if len(kmf.survival_function_) > 0:
                end_time = kmf.survival_function_.index[-1]
            # 取 survival_function_ 第一列的值（通常只有一列）
                end_prob = kmf.survival_function_.iloc[-1, 0]
                ax.annotate(
                    f'Bug at {pp_time_second(end_time)}',
                    xy=(end_time, end_prob),
                    xytext=(end_time, end_prob + 0.05),
                    arrowprops=dict(facecolor=fuzzer_colors[fuzzer], arrowstyle='->'),
                    fontsize=8
                )
